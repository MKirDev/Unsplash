...
import com.mkirdev.unsplash.details.api.navigation.DetailsDestination
            ...
            viewModel.applyEffect(function = { effect ->
                when (effect) {
                    is DetailsContract.Effect.Location -> {
                        ExternalIntentHelper.startMapIntent(
                            context = context,
                    ...
                    is DetailsContract.Effect.Share -> {
                        ExternalIntentHelper.startShareIntent(
                            context,
                            link = effect.link
                        )
                    }
            ...
            PhotoDetailsScreenWrapper(
                ...
                onRemoveLikeClick = {
                    viewModel.handleEvent(
                        DetailsContract.Event.PhotoUnlikedEvent(
                            it
                        )
                    )
                },
                onLocationClick = {
                    viewModel.handleEvent(
                        DetailsContract.Event.LocationOpenedEvent(
                            it
                        )
                    )
                },
                onDownloadClick = {
                    viewModel.handleEvent(
                        DetailsContract.Event.DownloadRequestedEvent(
                            it
                        )
                    )
                },
